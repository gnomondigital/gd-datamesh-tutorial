...
from gd_nasa_pipeline.utils.config import open_data_config
...
class PostgresHandler:
    ...
    def write_dataframe(
        ...
        cur = conn.cursor()
        output = io.StringIO()
        dataset.astype(str).to_csv(output, sep="\t", header=True, index=False)
        output.seek(0)
        insert_query = (
    ...
    def launch_connection_db(self, database_name: str):
        """create a connection"""
        conn_type = f"postgresql+psycopg2://{self.user}:{self.password}"
        conn_string = f"{conn_type}@{self.url}:{self.port}/{database_name}"
        engine = create_engine(conn_string)
